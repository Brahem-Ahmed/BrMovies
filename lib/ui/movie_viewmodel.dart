...
import 'package:br_movies/data/models/genre.dart';
...
import 'package:br_movies/ui/screens/movie_detail/movie_detail.dart';
...
class MovieViewModel {
  ...
  Future setupGenres() async {
    ...
    if (response.statusCode == 200) {
      movieGenres =
          Genres.fromJson(response.data).genres;
    } else {
      logError(
  ...
  void updateFavorite(Favorite favorite) {
    ...
    element.movieId == favorite.movieId);
    if (index != -1) {
...
// 5
  ...
  Future<MovieResponse> getNowPlaying(int page) async {
   ...
   if ( response.statusCode == 200) {
    ...
    logError(    'Failed to load movies with error ${response.statusCode} and message ${response.statusMessage}');
    throw Exception('Failed to load movies');
    }
   }
